...
const ArticleCard = ({ 
  ...
  const handleLike = async (e) => {
    ...
    if (!user) {
      toast.error(t('errors.pleaseLoginToLikeArticles'));
      return;
    }
    ...
    try {
      ...
      toast.error(t('messages.failedToUpdateLikeStatus'));
    } finally {
  ...
  const handleDislike = async (e) => {
    ...
    if (!user) {
      toast.error(t('errors.pleaseLoginToInteractWithArticles'));
      return;
    }
  ...
  const handleBookmark = async (e) => {
    ...
    if (!user) {
      toast.error(t('errors.pleaseLoginToBookmarkArticles'));
      return;
    }
    ...
    try {
      ...
      if (isBookmarked) {
        ...
        if (response.success) {
          ...
          toast.success(t('messages.bookmarkRemoved'));
        }
      } else {
        const response = await userApi.bookmarkArticle(articleId);
        if (response.success) {
          setIsBookmarked(true);
          toast.success(t('messages.articleBookmarked'));
        }
      }
    } catch (error) {
      toast.error(t('messages.failedToUpdateBookmark'));
    }
  };
            ...
            <div className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-3" onClick={handleAuthorClick}>
                  ...
                  <div>
                    <p className="font-medium text-gray-900 cursor-pointer hover:text-blue-600">
                      {article.author_name || article.author?.name || article.author?.full_name || t('articleCard.unknownAuthor')}
                    </p>
                    <p className="text-sm text-gray-500">
                      {(article.created_date || article.created_at) ? formatDistanceToNow(new Date(article.created_date || article.created_at), { addSuffix: true }) : t('articleCard.recently')}
                    </p>
                  </div>
                </div>
              )}
              ...
              <div className="flex items-center space-x-6 text-sm text-gray-500">
                <span className="flex items-center">
                  ...
                  {calculateReadingTime(contentForReading)} {t('articleCard.min')}
                </span>
                <span className="flex items-center">
                ...
                <div className="flex items-center space-x-2" onClick={handleAuthorClick}>
                  ...
                  <div>
                    <p className="text-sm font-medium text-gray-900 cursor-pointer hover:text-blue-600">
                      {article.author_name || article.author?.full_name || t('articleCard.unknownAuthor')}
                    </p>
