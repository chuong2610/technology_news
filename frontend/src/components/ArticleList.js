...
const ArticleList = ({ 
  ...
  const [pagination, setPagination] = useState({
    current: 1,
    pageSize: 12,
    // default to 5 pages * 12 pageSize (4 articles per row x 3 rows)
    total: 5 * 12
  });
        ...
        {articles.length === 0 && !isLoading ? (
          <div className="text-center py-16">
            ...
            <h3 className="text-lg font-medium text-gray-900 mb-2">
              {searchQuery || searchText ? 'No articles found' : 'No articles available yet'}
            </h3>
            <p className="text-gray-500 mb-4">
              {searchQuery || searchText 
                ? 'Try adjusting your search criteria or check back later.'
                : 'Be the first to contribute! Start by creating an article to share your insights.'
              }
            </p>
            {!(searchQuery || searchText) && (
              <div className="space-y-2">
                ...
                <p className="text-sm text-gray-400">Help build this community by sharing your knowledge</p>
              </div>
            )}
          </div>
        ) : (
