		...
		<div className="absolute -right-10 -bottom-10 h-40 w-40 rounded-full" style={{ background: 'rgba(255,255,255,0.04)', filter: 'blur(24px)' }} />
			<div className="relative flex flex-col items-center sm:items-start gap-4 sm:flex-row sm:items-center sm:justify-between">
				<div className="text-center sm:text-left">
					<h2 className="text-xl sm:text-2xl lg:text-3xl font-bold">{t('home.newsletter.title')}</h2>
					<p className="mt-1 text-white/90 text-sm sm:text-base">{t('home.newsletter.subtitle')}</p>
				</div>
				<form className="mt-3 flex flex-col sm:flex-row w-full max-w-md gap-2 sm:mt-0">
					<input type="email" placeholder={t('home.newsletter.emailPlaceholder')} className="w-full rounded-full px-4 py-2 sm:py-3 text-black placeholder:text-gray-500 focus:outline-none focus:ring-2 focus:ring-white text-sm sm:text-base" />
					<button type="submit" className="rounded-full bg-white px-4 sm:px-6 py-2 sm:py-3 font-medium text-blue-700 transition hover:bg-blue-50 text-sm sm:text-base whitespace-nowrap">{t('home.newsletter.subscribe')}</button>
				</form>
			</div>
		</section>
	);
