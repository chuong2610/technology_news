...
const DashboardStats = ({ userId, className = "" }) => {
  ...
  const primaryMetrics = [
    {
      title: t('dashboard.writingStatistics.totalArticles'),
      value: stats.total_articles || 0,
      icon: DocumentSolid,
      color: 'from-blue-500 to-indigo-600',
      bgColor: 'from-blue-50 to-indigo-50',
      description: t('dashboard.writingStatistics.totalArticlesDesc')
    },
    {
      title: t('dashboard.writingStatistics.published'),
      value: stats.published_articles || 0,
      icon: BookOpenSolid,
      color: 'from-green-500 to-emerald-600',
      bgColor: 'from-green-50 to-emerald-50',
      description: t('dashboard.writingStatistics.publishedDesc')
    },
    {
      title: t('dashboard.writingStatistics.totalViews'),
      value: formatNumber(stats.total_views || 0),
      icon: EyeSolid,
      color: 'from-purple-500 to-indigo-600',
      bgColor: 'from-purple-50 to-indigo-50',
      description: t('dashboard.writingStatistics.totalViewsDesc')
    },
    {
      title: t('dashboard.writingStatistics.totalLikes'),
      value: formatNumber(stats.total_likes || 0),
      icon: HeartSolid,
      color: 'from-pink-500 to-red-500',
      bgColor: 'from-pink-50 to-red-50',
      description: t('dashboard.writingStatistics.totalLikesDesc')
    }
  ];
  ...
  const secondaryMetrics = [
    {
      title: t('dashboard.performanceInsights.publishRate'),
      value: `${publishRate}%`,
      icon: ArrowTrendingUpIcon,
      color: 'from-blue-500 to-cyan-500',
      bgColor: 'from-blue-50 to-cyan-50',
      description: t('dashboard.performanceInsights.publishRateDesc')
    },
    {
      title: t('dashboard.performanceInsights.avgViews'),
      value: formatNumber(avgViews),
      icon: EyeIcon,
      color: 'from-emerald-500 to-green-500',
      bgColor: 'from-emerald-50 to-green-50',
      description: t('dashboard.performanceInsights.avgViewsDesc')
    },
    {
      title: t('dashboard.performanceInsights.avgLikes'),
      value: formatNumber(avgLikes),
      icon: HeartIcon,
      color: 'from-rose-500 to-pink-500',
      bgColor: 'from-rose-50 to-pink-50',
      description: t('dashboard.performanceInsights.avgLikesDesc')
    }
  ];
      ...
      {/* Writing Statistics Dashboard */}
          ...
          <div className="bg-gradient-to-r from-indigo-50 to-purple-50 px-4 sm:px-6 lg:px-8 py-4 sm:py-6 border-b border-slate-200">
            <h2 className="text-xl sm:text-2xl font-bold text-slate-900 flex items-center gap-3">
              ...
              {t('dashboard.writingStatistics.title')}
            </h2>
            <p className="text-slate-600 mt-2 text-sm sm:text-base">
              {t('dashboard.writingStatistics.description')}
            </p>
          </div>
        ...
        <div className="p-4 sm:p-6 lg:p-8">
          ...
          <div className="mb-8">
            <h3 className="text-lg font-semibold text-slate-700 mb-4 flex items-center gap-2">
              ...
              {t('dashboard.performanceInsights.title')}
            </h3>
            <motion.div 
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              >
                ...
                <div>
                  <div className="font-medium text-slate-700">{t('dashboard.quickActions.createArticle')}</div>
                  <div className="text-xs text-slate-500">{t('dashboard.quickActions.createArticleDesc')}</div>
                </div>
              </button>
              <button 
                onClick={() => navigate('/write')}
                className="flex items-center gap-3 p-3 bg-white rounded-lg border border-slate-200 hover:border-green-300 hover:shadow-md transition-all duration-200 text-left w-full"
              >
                <div className="w-8 h-8 bg-green-100 rounded-lg flex items-center justify-center">
                  <DocumentTextIcon className="w-4 h-4 text-green-600" />
                </div>
                <div>
                  <div className="font-medium text-slate-700">{t('dashboard.quickActions.generateArticle')}</div>
                  <div className="text-xs text-slate-500">{t('dashboard.quickActions.generateArticleDesc')}</div>
                </div>
              </button>
              <button 
              ...
              >
                ...
                <div>
                  <div className="font-medium text-slate-700">{t('dashboard.quickActions.publishDraft')}</div>
                  <div className="text-xs text-slate-500">{t('dashboard.quickActions.publishDraftDesc')}</div>
                </div>
              </button>
              <button 
              ...
              >
                ...
                <div>
                  <div className="font-medium text-slate-700">{t('dashboard.quickActions.viewAnalytics')}</div>
                  <div className="text-xs text-slate-500">{t('dashboard.quickActions.viewAnalyticsDesc')}</div>
                </div>
              </button>
            </div>
          </motion.div>
