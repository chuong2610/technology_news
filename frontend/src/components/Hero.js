		...
		<section ref={ref} className="relative overflow-hidden bg-gradient-to-br from-indigo-50 via-white to-purple-50" style={{ background: 'linear-gradient(135deg, rgba(59,130,246,0.03) 0%, var(--bg) 50%, rgba(236,72,153,0.02) 100%)' }}>
			...
			<div className="relative mx-auto max-w-7xl px-4 sm:px-6 py-16 sm:py-20 lg:flex lg:items-center lg:gap-16 lg:py-32">
				...
				>
					...
					>
						<span className="block bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 bg-clip-text text-transparent">{t('home.hero.title')}</span>
						<span className="block text-indigo-700">{t('home.hero.titleSuffix')}</span>
					</motion.h1>
					...
					>
						...
						>
							<span className="relative z-10">{t('home.hero.writeArticle')}</span>
			...
			>
				<div className="text-center mb-6 sm:mb-8">
					<h2 className="text-xl sm:text-2xl font-bold text-gray-900 mb-2">{t('home.categories.title')}</h2>
					<p className="text-gray-600 text-sm sm:text-base">{t('home.categories.allCategories')}</p>
				</div>
