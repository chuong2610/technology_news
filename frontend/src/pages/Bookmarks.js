  ...
  const fetchBookmarkedArticles = async () => {
    try {
      ...
      console.error('Error fetching bookmarked articles:', error);
            message.error(t('errors.failedToLoadBookmarks'));
      setBookmarkedArticles([]);
    } finally {
      setLoading(false);
    }
        ...
        <div className="max-w-4xl mx-auto px-6 py-12">
          ...
          >
            <div className="flex items-center justify-center mb-6">
              ...
              <h1 className="text-4xl font-bold text-gray-900">{t('bookmarks.title')}</h1>
            </div>
            <p className="text-xl text-gray-600">{t('bookmarks.subtitle')}</p>
          </motion.div>
          ...
          >
            ...
            <h2 className="text-2xl font-bold text-gray-900 mb-4">{t('bookmarks.emptyState.title')}</h2>
            ...
            <div className="bg-gray-50 rounded-xl p-6 max-w-sm mx-auto">
              <div className="flex items-center justify-center mb-3">
                ...
                <span className="text-sm text-gray-600">{t('bookmarks.emptyState.instruction')}</span>
              </div>
              <p className="text-xs text-gray-500">{t('bookmarks.emptyState.note')}</p>
            </div>
          </motion.div>
        </div>
      </div>
