...
const Dashboard = () => {
  ...
  const fetchUsers = async (page = 1, pageSize = 20) => {
    try {
      ...
      if (response.success) {
        ...
        message.error(t('errors.failedToFetchUsers'));
  ...
  const handleUpdateUser = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        message.error(t('errors.failedToUpdateUser'));
      }
    } catch (error) {
      message.error(t('errors.failedToUpdateUser'));
  ...
  const confirmDeleteUser = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        message.error(t('errors.failedToDeleteUser'));
      }
    } catch (error) {
      message.error(t('errors.failedToDeleteUser'));
  ...
  const handleReactivateUser = async (user) => {
    try {
      ...
      if (response.success) {
        message.success(t('messages.userReactivatedSuccessfully'));
        // Reload current page instead of all users
        fetchUsers(pagination.current, pagination.pageSize);
      } else {
        message.error(t('errors.failedToReactivateUser'));
      }
    } catch (error) {
      message.error(t('errors.failedToReactivateUser'));
        ...
        <div className="space-y-2">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Tag color={getStatusColor(record.is_active)} className="text-xs">
              {record.is_active !== false ? t('dashboard.active') : t('dashboard.inactive')}
            </Tag>
          </div>
        </div>
      ),
      ...
      render: (_, record) => (
        <div className="space-y-1 text-sm">
          <div className="flex justify-between">
            <span className="text-gray-500">{t('dashboard.articles')}:</span>
            <span className="font-medium">{record.articles_count || 0}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-gray-500">{t('dashboard.views')}:</span>
            <span className="font-medium">{formatNumber(record.total_views || 0)}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-gray-500">{t('dashboard.joined')}:</span>
        ...
        <Dropdown
          menu={{
            items: [
              {
                key: 'edit',
                label: t('dashboard.editUser'),
                icon: <CogIcon className="w-4 h-4" />,
                onClick: () => handleEditUser(record)
              },
              ...(record.is_active === false && record.id !== user?.id ? [{
                key: 'reactivate',
                label: t('dashboard.reactivate'),
      ...
      render: (isActive, record) => (
        <div className="flex items-center space-x-2">
          <Tag color={getStatusColor(isActive)}>
            {isActive !== false ? t('dashboard.active') : t('dashboard.inactive')}
          </Tag>
          {isActive === false && record.id !== user?.id && (
            <Button
              ...
              onClick={() => handleReactivateUser(record)}
              title={t('dashboard.reactivateUser')}
            >
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-blue-50 to-indigo-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 lg:py-12">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              <div>
                ...
                <p className="text-gray-600 mt-2 text-sm sm:text-base">
                  {t('dashboard.userManagementSubtitle')}
                </p>
              </div>
              <Button 
                type="primary" 
              ...
              {isMobile && (
                <div className="mb-4">
                  <Button
                    ...
                    className="w-full"
                  >
                    {t('dashboard.showFiltersAndSearch')}
                  </Button>
                </div>
              )}
                ...
                <>
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div>
                      ...
                      >
                        ...
                        <Option value="writer">{t('dashboard.writer')}</Option>
                    ...
                    <div>
                      <Select
                        ...
                        className="w-full"
                        placeholder={t('dashboard.filterByStatus')}
                      >
                  ...
                  <div className="flex justify-between items-center mt-4">
                    <div className="text-sm text-gray-600">
                      {t('dashboard.showingUsers')} <span className="font-semibold">{filteredUsers.length}</span> {t('dashboard.usersOnThisPage')}
                      <span className="ml-2">
                        ({t('dashboard.pageOf')} {pagination.current} {t('dashboard.of')} {Math.ceil(pagination.total / pagination.pageSize)})
                      </span>
                      <span className="ml-2">
                        • {t('dashboard.total')}: <span className="font-semibold">{pagination.total}</span> {t('dashboard.users')}
                      </span>
                      {searchText && (
                        <span className="ml-2">
                          • {t('dashboard.search')}: "<span className="font-medium">{searchText}</span>"
                        </span>
                      )}
                      {roleFilter !== 'all' && (
                        <span className="ml-2">
                          • {t('dashboard.role')}: <span className="font-medium capitalize">{roleFilter}</span>
                        </span>
                      )}
                      {statusFilter !== 'all' && (
                        <span className="ml-2">
                          • {t('dashboard.status')}: <span className="font-medium capitalize">{statusFilter}</span>
                        </span>
                      )}
                    </div>
              ...
              {isMobile && (
                <div className="text-sm text-gray-600 text-center">
                  ...
                  <span className="block mt-1">
                    {t('dashboard.page')} {pagination.current} {t('dashboard.of')} {Math.ceil(pagination.total / pagination.pageSize)} • {t('dashboard.total')}: {pagination.total}
                  </span>
                  {(searchText || roleFilter !== 'all' || statusFilter !== 'all') && (
                    <span className="block mt-1">
                      {t('dashboard.filtersApplied')} • <Button 
                        type="link" 
                        onClick={clearFilters}
                        size="small"
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex flex-wrap gap-2">
                        <Tag color={getRoleColor(userRecord.role)} className="capitalize">
                          {userRecord.role || t('dashboard.user')}
                        </Tag>
                        <Tag color={getStatusColor(userRecord.is_active)}>
                          {userRecord.is_active !== false ? t('dashboard.active') : t('dashboard.inactive')}
                        </Tag>
                      </div>
                ...
                <div className="flex justify-center mt-6">
                  <div className="flex space-x-2">
                    ...
                    <span className="px-3 py-2 text-sm text-gray-600">
                      {t('dashboard.page')} {pagination.current} {t('dashboard.of')} {Math.ceil(pagination.total / pagination.pageSize)}
                    </span>
                    <Button
                      size="small"
              ...
              <Table
                ...
                pagination={{
                  ...
                  showTotal: (total, range) => 
                    `${range[0]}-${range[1]} ${t('dashboard.of')} ${total} ${t('dashboard.users')}`,
        ...
        {/* Edit User Modal */}
        <Modal
          title={t('dashboard.editUserTitle', { name: selectedUser?.full_name })}
          open={editModalVisible}
          onOk={handleUpdateUser}
          onCancel={() => setEditModalVisible(false)}
          confirmLoading={updating}
          okText={t('dashboard.updateUser')}
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <Select
                ...
                className="w-full"
              >
                <Option value={true}>{t('dashboard.active')}</Option>
                <Option value={false}>{t('dashboard.inactive')}</Option>
              </Select>
              <p className="text-xs text-gray-500 mt-1">
                {t('dashboard.statusNote')}
              </p>
            </div>
              ...
              <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                <div className="flex items-center">
                  ...
                  <span className="text-sm text-yellow-800">
                    {t('dashboard.warningCannotDeactivate')}
                  </span>
                </div>
              </div>
            )}
        ...
        <Modal
          title={t('dashboard.deleteUserTitle')}
        ...
        >
          <div className="space-y-4">
            ...
            <div className="text-gray-700">
              <p>{t('dashboard.deleteConfirmation', { name: selectedUser?.full_name })}</p>
        ...
        <Drawer
          title={t('dashboard.searchAndFilters')}
          placement="right"
          ...
          <div className="space-y-6">
            ...
            {(searchText || roleFilter !== 'all' || statusFilter !== 'all') && (
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                <h4 className="text-sm font-medium text-blue-800 mb-2">{t('dashboard.activeFilters')}</h4>
                <div className="space-y-2 text-sm text-blue-700">
                  {searchText && (
                    <div>• {t('dashboard.search')}: "{searchText}"</div>
                  )}
                  {roleFilter !== 'all' && (
                    <div>• {t('dashboard.role')}: {roleFilter}</div>
                  )}
                  {statusFilter !== 'all' && (
                    <div>• {t('dashboard.status')}: {statusFilter}</div>
                  )}
                </div>
              </div>
            )}
            ...
            <div className="text-center text-sm text-gray-600 pt-4 border-t border-gray-200">
              {t('dashboard.showingUsers')} <span className="font-semibold">{filteredUsers.length}</span> {t('dashboard.usersOnThisPage')}
              <div className="mt-1">
                {t('dashboard.page')} {pagination.current} {t('dashboard.of')} {Math.ceil(pagination.total / pagination.pageSize)} • {t('dashboard.total')}: <span className="font-semibold">{pagination.total}</span> {t('dashboard.users')}
              </div>
            </div>
          </div>
        </Drawer>
