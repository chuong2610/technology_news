...
const Profile = () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        if (response.error === 'account_deleted') {
          ...
          setErrorMessage(response.message || t('errors.thisAccountHasBeenDeleted'));
          setUser(null);
          ...
          throw new Error(response.error || t('errors.failedToFetchUser'));
        }
      }
    } catch (error) {
      message.error(t('errors.failedToLoadUserInfo'));
      console.error('Error fetching user:', error);
      setErrorType('general_error');
      setErrorMessage(t('errors.failedToLoadUserInfo'));
      setUser(null);
    } finally {
      setLoading(false);
    }
  ...
  const handleShowFollowing = async () => {
    ...
    if (followingArr.length === 0) {
      message.info(t('profile.notFollowingAnyone'));
      return;
    }
    ...
    try {
      ...
      message.error(t('profile.failedToLoadFollowingList'));
    } finally {
  ...
  const handleShowFollowers = async () => {
    ...
    if (followersArr.length === 0) {
      if (stats.followers > 0) {
        message.info(t('profile.hasFollowersButListNotAvailable'));
      } else {
        message.info(t('profile.noFollowers'));
      }
      return;
    }
    ...
    try {
      ...
      message.error(t('profile.failedToLoadFollowersList'));
    } finally {
  ...
  const handleFollow = async () => {
    if (!isAuthenticated()) {
      message.warning(t('errors.pleaseLoginToFollow'));
      return;
    }
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 flex items-center justify-center">
        <div className="bg-white rounded-3xl p-12 shadow-2xl border border-slate-200 text-center max-w-2xl mx-4">
          ...
          <h1 className="text-3xl font-bold text-slate-900 mb-4">{t('profile.errorLoadingProfile')}</h1>
          <p className="text-slate-600 text-lg mb-8">{errorMessage}</p>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100">
            ...
            <div className="text-center lg:text-left flex-1">
              ...
              {(user?.created_at || user?.joined) && (
                <div className="flex items-center justify-center lg:justify-start gap-2 text-blue-100 mb-4">
                  ...
                  <span className="text-lg">
                    {t('profile.joined')} {new Date(user.created_at || user.joined).toLocaleDateString('en-US', {
                      year: 'numeric',
                      month: 'long', 
                      day: 'numeric'
                    })}
              ...
              >
                ...
                <span className="text-lg font-semibold">
                  {stats.followers} {t('profile.followersLabel')}
                </span>
              </div>
              ...
              <div className="flex gap-4 justify-center lg:justify-start">
                {currentUser?.id === user?.id ? (
                  <button
                    onClick={() => message.info(t('profile.editProfileFunctionality'))}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-16">
          <div className="bg-white rounded-3xl p-8 shadow-xl border border-slate-200 text-center hover:shadow-2xl transition-shadow duration-300">
            ...
            <div className="text-slate-600 font-semibold">{t('profile.articlesLabel')}</div>
          </div>
          ...
          <div className="bg-white rounded-3xl p-8 shadow-xl border border-slate-200 text-center hover:shadow-2xl transition-shadow duration-300">
            ...
            <div className="text-slate-600 font-semibold">{t('profile.views')}</div>
          </div>
          ...
          <div className="bg-white rounded-3xl p-8 shadow-xl border border-slate-200 text-center hover:shadow-2xl transition-shadow duration-300">
            ...
            <div className="text-slate-600 font-semibold">{t('profile.likes')}</div>
          </div>
          ...
          >
            ...
            <div className="text-slate-600 font-semibold">{t('profile.followingLabel')}</div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-3xl shadow-xl border border-slate-200 overflow-hidden">
          <div className="bg-gradient-to-r from-indigo-50 to-purple-50 px-8 py-6 border-b border-slate-200">
            <h2 className="text-3xl font-bold text-slate-900 flex items-center gap-3">
              ...
              {(user?.name || user?.full_name || user?.username) ? t('profile.articlesTitle', { name: user.name || user.full_name || user.username }) : t('profile.articlesLabel')}
            </h2>
            <p className="text-slate-600 text-lg mt-2">
              {stats.totalArticles > 0 
                ? t('profile.discoverArticles', { 
                    count: stats.totalArticles, 
                    articles: stats.totalArticles === 1 ? t('profile.article') : t('profile.articles')
                  })
      ...
      {/* Followers Modal */}
      <Modal
        title={t('profile.followersModal', { name: user?.name || 'User' })}
      ...
      {/* Following Modal */}
      <Modal
        title={t('profile.followingModal', { name: user?.name || 'User' })}
